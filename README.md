...
|---|---|
| Laravel | 11, 12 |
| Symfony | 6.4, 7.x, 8.x (avec DoctrineBundle 2.13+ ou 3.x) |
...
Les conditions historiques des datatables (`contain`, `start_with`, `end_with`, `equal`) sont acceptées telles quelles comme mode :
...
Chaque entrée ajoute un `REPLACE()` à la requête : n'ajoutez que les caractères présents dans vos données.
